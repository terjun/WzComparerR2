...
namespace WzComparerR2.CharaSimControl
{
    public class GearTooltipRender : TooltipRender
    {
        ...
        private Bitmap renderBase(out int picHeight, out int iconY)
        {
            ...
            g.DrawString("新手", GearGraphics.EquipDetailFont, reqJob > 0 ? Brushes.Red : Brushes.White, 10, picHeight);
            if (reqJob == 0) reqJob = 0x1f;//0001 1111
            if (reqJob == -1) reqJob = 0; //0000 0000
            g.DrawString("战士", GearGraphics.EquipDetailFont, (reqJob & 1) == 0 ? Brushes.Red : Brushes.White, 46, picHeight);
            g.DrawString("魔法师", GearGraphics.EquipDetailFont, (reqJob & 2) == 0 ? Brushes.Red : Brushes.White, 82, picHeight);
            g.DrawString("弓箭手", GearGraphics.EquipDetailFont, (reqJob & 4) == 0 ? Brushes.Red : Brushes.White, 130, picHeight);
            g.DrawString("飞侠", GearGraphics.EquipDetailFont, (reqJob & 8) == 0 ? Brushes.Red : Brushes.White, 178, picHeight);
            g.DrawString("海盗", GearGraphics.EquipDetailFont, (reqJob & 16) == 0 ? Brushes.Red : Brushes.White, 214, picHeight);
            picHeight += 19;
            ...
            if (hasTuc)
            {
                g.DrawString("·可升级次数 : " + value + "回", GearGraphics.EquipDetailFont, Brushes.White, 8, picHeight);
                picHeight += 16;
                hasPart2 = true;
            }
            if (gear.Props.TryGetValue(GearPropType.limitBreak, out value) && value > 0)
            ...
            if (hasTuc && gear.Hammer > -1)
            {
                if (gear.Hammer == 2)
                {
                    g.DrawString("黄金锤提炼完成", GearGraphics.EquipDetailFont, Brushes.White, 8, picHeight);
                    picHeight += 16;
                }
                if (gear.Props.TryGetValue(GearPropType.superiorEqp, out value) && value > 0)
                {
                ...
                if (gear.Star > 0)
                {
                    g.DrawString("·应用" + gear.Star + "星强化", GearGraphics.EquipDetailFont, GearGraphics.OrangeBrush, 8, picHeight);
                    picHeight += 16;
                }
                picHeight += 2;
                g.DrawString("金锤子已提高的强化次数", GearGraphics.EquipDetailFont, GearGraphics.GoldHammerBrush, 8, picHeight);
            ...
            if (adOptionCount > 0)
            {
                ...
                g.DrawString("附加潜能", GearGraphics.EquipDetailFont, GearGraphics.SetItemNameBrush, 26, picHeight+2);
                picHeight += 24;
        ...
        private Bitmap renderSetItem(out int picHeight)
        {
            ...
            if (gear.Props.TryGetValue(GearPropType.setItemID, out setID))
            {
                ...
                {
                    g.DrawString(effect.Key + "套装效果", GearGraphics.EquipDetailFont, GearGraphics.SetItemNameBrush, 8, picHeight);
                    picHeight += 16;
